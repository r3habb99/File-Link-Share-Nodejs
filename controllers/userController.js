...
//       email: email,
//       password: hashedPassword,
//       active: false, // Set the active flag to false initially
//     });

//     // Generate a verification token and save it to the database
//     const token = crypto.randomBytes(32).toString("hex"); // Generate a random 32-byte hex string
//     const verificationToken = new VerificationToken({
//       userId: user._id,
//       token: token,
//     });
...
//     const verificationLink = `${req.protocol}://${req.get(
//       "host"
//     )}/api/users/verify/${user._id}/${token}`; // Construct the verification link from the request origin and the user id and token
//     const mailOptions = successfulRegister(user, verificationLink); // Call the predefined function to create the mail options object
